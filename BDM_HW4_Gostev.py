...
if __name__=='__main__':
    ...
    places = sc.textFile('hdfs:///data/share/bdm/core-places-nyc.csv')         .map(lambda x: x.split(','))         .map(lambda x: (x[1], x[9]))         .filter(lambda x:  x[1] in NAICS_dict.keys())         .map(lambda x: addCategory(x))         .collect()
    ...
    def MedLowHigh(x):
        ...
        patterns = sc.textFile('hdfs:///data/share/bdm/weekly-patterns-nyc-2019-2020.csv')             .map(lambda x: next(csv.reader([x])))             .filter(lambda x: x[1] in places_dict.keys())             .map(lambda x: (x[1], x[12], x[16] ))             .flatMap(lambda x: convertKey(x))             .groupByKey()             .map(lambda x: (x[0], list(x[1]) ))             .map(lambda x: MedLowHigh(x))             .filter(lambda x: x[0] > 2018)             .sortBy(lambda x: (x[0], x[1]))             .collect() 
        schema = StructType([             StructField("year",StringType(),True),             StructField("date",StringType(),True),             StructField("median",IntegerType(),True),             StructField("low", IntegerType(), True),             StructField("high", IntegerType(), True),             StructField("type", StringType(), True)])
