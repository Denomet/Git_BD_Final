# -*- coding: utf-8 -*-
"""BDM_HW4_Gostev.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1ouOTRrMSCdMGdf2YWYCJzAfQmf6vpMwr
"""
...
#         .collect()
    ...
    header_rdd.saveAsTextFile(sys.argv[1]+'/type=drinking_places/part-0.csv' if len(sys.argv)>1 else '/test/type=drinking_places/part-0.csv')
    header_rdd.saveAsTextFile(sys.argv[1]+'/type=full_service_restaurants/part-0.csv' if len(sys.argv)>1 else '/test/type=full_service_restaurants/part-0.csv')
    header_rdd.saveAsTextFile(sys.argv[1]+'/type=limited_service_restaurants/part-0.csv' if len(sys.argv)>1 else '/test/type=limited_service_restaurants/part-0.csv')
    header_rdd.saveAsTextFile(sys.argv[1]+'/type=pharmacies_and_drug_stores/part-0.csv' if len(sys.argv)>1 else '/test/type=pharmacies_and_drug_stores/part-0.csv')
    header_rdd.saveAsTextFile(sys.argv[1]+'/type=snack_and_bakeries/part-0.csv' if len(sys.argv)>1 else '/test/type=snack_and_bakeries/part-0.csv')
    header_rdd.saveAsTextFile(sys.argv[1]+'/type=specialty_food_stores/part-0.csv' if len(sys.argv)>1 else '/test/type=specialty_food_stores/part-0.csv')
    header_rdd.saveAsTextFile(sys.argv[1]+'/type=supermarkets_except_convenience_stores/part-0.csv' if len(sys.argv)>1 else '/test/type=supermarkets_except_convenience_stores/part-0.csv')
