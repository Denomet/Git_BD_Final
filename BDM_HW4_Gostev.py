...
#===================
    ...
    def extractVisits(storeGroup, _, lines):
      ...
      reader = csv.reader(lines)
      for line in reader:
        if line[12] >= '2018-12-23T00:00:00-00:00' and line[12] < '2021-01-00T00:00:00-00:00':
          if line[0] in storeGroup:
            group = storeGroup[line[0]]
            for i in range(7):
                date =(datetime.datetime.strptime(line[12][:10], '%Y-%m-%d').date() + datetime.timedelta(days=i)).strftime('%Y-%m-%d')
...
#Print output files
    ...
    rddJ.filter(lambda x: x[0]==0 or x[0]==-1).values() \
        .saveAsTextFile(OUTPUT_PREFIX+'/'+filename)
